...
head(ap)
x<-as.factor(ap[,9]) # independent variable (factor)
x1 <-as.factor(ap[,12])
...
## Run PCA first
#----
pca<-princomp(y,cor=T) # total length not same unit of measure, thus scale all to Z-score
...
par(mfrow=c(1,2))
plot(pca$scores[,1], pca$scores[,2],pch=as.numeric(ap$sex),col=c("red","blue")[x1], xlab="PC 1(41%)", ylab="PC 2(32%)") #eigenvalues for each axis
...
plot(pca$scores[,1], pca$scores[,2],pch=as.numeric(ap$form),col=c(1,2,3,4)[x],xlab="PC 1 (41%)", ylab="PC 2(32%)") #eigenvalues for each axis
...
ap.fit<-ap[-c(11,18,35,72, 32:33),] # removed sites to improve multinormality-skewed (cooks.r)
# 32 & 33 removed due to sample size of 2
x.fit<-as.factor(ap.fit[,9]) # independent variable (factor)
y.fit<-as.matrix(ap.fit[,c(2:7)])
mod.fit<-manova(y.fit~x.fit)

## test the normality assumption again
chisplot(resid(mod.fit))
mshapiro.test(t(resid(mod.fit)))  # small size issue!

### hypothesis test, alternative to ANOVA
library(vegan)
adonis(y ~ form, data=env, perm=999)
...
names<-colnames(y.fit)
for (i in 1:ncol(y)){
  boxplot(y.fit[,i]~x.fit,main=paste("Boxplot of",names[i]),xlab="Group", ylab=paste(names[i]))
}
...
m<-betadisper(dist(y.fit),ap.fit$form, type="centroid") #calculate average distance to its group centroid for each group
...
m.Log<-betadisper(dist(log(y.fit)+1),ap.fit$form, type="centroid") #calculate average distance to its group centroid for each group
...
tb<-table(Predicted=predict(dfa,ap.fit[,-c(1,8:12)],type='class')$class,Observed=ap.fit[,"form"])
